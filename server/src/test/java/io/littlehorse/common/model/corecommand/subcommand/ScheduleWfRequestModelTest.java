...
class ScheduleWfRequestModelTest {
    ...
    void shouldScheduleFirstRunAtNextCronDate(String cronExpression, LocalDateTime expectedDate) {
        ...
        LocalDateTime capturedMaturationTime = LocalDateTime.ofInstant(capturedTimer.maturationTime.toInstant(), ZoneId.systemDefault());
    ...
    private static Stream<Arguments> scheduleFirstRunCases() {
        return Stream.of(
                Arguments.of(
                        "5 4 10 2 *",
                        LocalDateTime.now().withDayOfMonth(10).withMonth(2).withHour(4).withMinute(5)),
                Arguments.of(
                        "0 12 1 6 *",
                        LocalDateTime.now().withDayOfMonth(1).withMonth(6).withHour(12).withMinute(0)),
                Arguments.of(
                        "30 15 * * *",
                ...
                Arguments.of(
                        "45 23 1 10 *",
                        LocalDateTime.now().withHour(23).withMinute(45).withDayOfMonth(1).withMonth(10)),
                Arguments.of(
                        "15 7 1 1 *",
                        LocalDateTime.now().withDayOfMonth(1).withMonth(1).withHour(7).withMinute(15)),
                Arguments.of(
                        "0 0 1 1 *",
                        LocalDateTime.now().withHour(0).withMinute(0).withMonth(1).withDayOfMonth(1)),
                Arguments.of(
                        "20 19 1 2 *",
                        LocalDateTime.now().withHour(19).withMinute(20).withMonth(2).withDayOfMonth(1)),
                Arguments.of(
                        "10 11 15 3 *",
                        LocalDateTime.now().withDayOfMonth(15).withMonth(3).withHour(11).withMinute(10)));
    }
}

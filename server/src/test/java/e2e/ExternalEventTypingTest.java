...
public class ExternalEventTypingTest {
    ...
    public Workflow getBasicExternalEventWorkflow() {
        return Workflow.newWorkflow("typed-external-event", thread -> {
            ...
            ExternalEventNodeOutput payload = thread.waitForEvent("typed-as-int").registeredAs(Integer.class);
    ...
    void shouldRejectStringPayloadForIntegerEvent() {
        ...
        assertThatThrownBy(() -> {
            client.putExternalEvent(PutExternalEventRequest.newBuilder()
                    .setExternalEventDefId(ExternalEventDefId.newBuilder().setName("typed-external-event"))
                    .setWfRunId(id).setContent(LHLibUtil.objToVarVal("not-an-integer")).build());
        }).matches(exn -> {
            return exn instanceof StatusRuntimeException && ((StatusRuntimeException) exn).getStatus().getCode() == Code.INVALID_ARGUMENT && exn.getMessage().toLowerCase().contains("typed-external-event");
        });
    }

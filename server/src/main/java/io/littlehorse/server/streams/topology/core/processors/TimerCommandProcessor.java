...
public class TimerCommandProcessor extends CommandProcessor {
    ...
    public TimerCommandProcessor(LHServerConfig config, LHServer server, MetadataCache metadataCache, TaskQueueManager globalTaskQueueManager, AsyncWaiters asyncWaiters) {
        super(config, server, metadataCache, globalTaskQueueManager, asyncWaiters);
    }

    @Override
    public void init(ProcessorContext<String, CommandProcessorOutput> ctx) {
        this.ctx = ctx;

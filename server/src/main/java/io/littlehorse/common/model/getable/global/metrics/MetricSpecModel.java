...
public class MetricSpecModel extends MetadataGetable<MetricSpec> {
    ...
    private List<AggregatorModel> aggregators;
    ...
    public void initFrom(Message proto, ExecutionContext context) {
        ...
        aggregators = p.getAggregatorsList().stream()
                .map(aggregator -> LHSerializable.fromProto(aggregator, AggregatorModel.class, context))
                .collect(Collectors.toList());
    }

    @Override
    public MetricSpec.Builder toProto() {
        MetricSpec.Builder out = MetricSpec.newBuilder().setId(id.toProto()).setCreatedAt(LHUtil.fromDate(createdAt));
        for (AggregatorModel aggregator : aggregators) {
            out.addAggregators(aggregator.toProto());
        }
        return out;
    }

    public List<AggregatorModel> getAggregators() {

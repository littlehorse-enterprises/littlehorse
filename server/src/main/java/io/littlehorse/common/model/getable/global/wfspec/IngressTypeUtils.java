    ...
    private static void applyExpectedType(
        ...
        switch (typeDef.getDefinedTypeCase()) {
            case INLINE_ARRAY_DEF:
                ...
                break;
            case INLINE_MAP_DEF:
                if (value.getValueType() == VariableValue.ValueCase.MAP) {
                    ...
                    if (mapType != null && value.getMap().getEntries() != null) {
                        for (MapModel.MapEntryModel entry : value.getMap().getEntries()) {
                            applyExpectedType(mapType.getKeyType(), entry.getKey(), metadataManager);
                            applyExpectedType(mapType.getValueType(), entry.getValue(), metadataManager);
                        }
                    }
                }
                break;

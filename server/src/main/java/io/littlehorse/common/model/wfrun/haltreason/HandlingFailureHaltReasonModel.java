...
@Slf4j
public class HandlingFailureHaltReasonModel
    extends LHSerializable<HandlingFailureHaltReason>
    ...
    public boolean isResolved(WfRunModel wfRunModel) {
        ...
        if (handlerThread.status == LHStatus.COMPLETED) {
            ...
            if (handledNode.type == NodeTypeCase.WAIT_THREADS) {
                ...
                for (WaitForThreadModel wft : handledNode
                    .getWaitThreadsRun()
                    .getThreads()) {
    ...
    public static HandlingFailureHaltReasonModel fromProto(
        HandlingFailureHaltReason proto
    ) {

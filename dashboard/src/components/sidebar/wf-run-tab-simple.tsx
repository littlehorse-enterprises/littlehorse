...
import { mockWorkflowRuns, TIME_RANGES } from "@/components/flow/mock-data"
...
export default function WfRunTabSimple() {
  const { wfRunIds } = useWorkflow()
  ...
  // Fetch workflow runs using SWR
  const { data: wfRuns } = useSWR(
    wfRunIds ? ['workflowRuns', wfRunIds, statusFilter] : null,
    async () => {
      if (!wfRunIds) return []
      // Convert WfRunId objects to strings
      const wfRunIdStrings = wfRunIds.map(id => id.id)
      return getWfRunsFromIds(wfRunIdStrings, statusFilter.length > 0 ? statusFilter : undefined)
    }
  )
    ...
    <div className="flex-1 overflow-y-auto p-2">
      <div className="flex items-center justify-between mb-2">
        <div className="text-xs text-[#656565]">
          Showing {wfRuns?.length} of {wfRunIds?.length} workflow runs

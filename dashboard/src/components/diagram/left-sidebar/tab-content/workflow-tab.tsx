...
function VariableDefComponent(threadVarDef: ThreadVarDef) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-md px-2 py-1">
      <div className={cn('w-fit rounded-md border-2 px-1 text-xs font-bold', varTypeColors[variableType])}>
        {variableType}
      </div>
      <div className="overflow-hidden text-sm text-nowrap text-ellipsis">{threadVarDef.varDef?.name}</div>
    </div>
  )
}
...
export default function WorkflowTab({ wfSpec }: WorkflowTabProps) {
  ...
  const threadSpecsWithVariableDefs = useMemo(
    () =>
      Object.entries(wfSpec.threadSpecs)
        .filter(threadSpec => threadSpec[1].variableDefs.length > 0)
        .sort(a => (a[0] === 'entrypoint' ? -1 : 1)),
    [wfSpec.threadSpecs]
  )

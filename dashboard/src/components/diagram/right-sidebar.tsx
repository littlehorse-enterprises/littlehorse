...
export default function RightSidebar({ wfSpec, nodeRuns, taskRuns }: RightSidebarProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tab Navigation */}
        <div className="sticky top-[73px] z-10 border-b border-gray-200 bg-white px-4 pt-2">
        ...
        <div className="flex-1 overflow-y-auto">
          {activeTab === 'runs' && (
            <div className="flex h-full flex-col p-4">
              <h3 className="mb-2 text-sm font-medium">NodeRuns</h3>

              {/* NodeRuns List */}
              <div className="h-fit max-h-1/3 overflow-y-auto">
              ...
              {selectedNodeRuns.length === 0 && (
                <div className="text-xs text-gray-500 italic">No runs found for this node</div>
              )}
                ...
                <div className="flex-shrink-0">
                  ...
                  <div>
                    ...
                    <h4 className="mb-2 text-xs font-medium">
                      Task Attempts ({selectedTaskRuns[0].attempts?.length || 0})
                    </h4>
                    ...
                    <Accordion
                      ...
                      collapsible
                      className="w-full"
                      defaultValue={selectedTaskRuns[0].id?.wfRunId?.id + '-' + 0}
                    >
                      {selectedTaskRuns[0].attempts?.map((attempt, index) => (
                        <AccordionItem
                          key={selectedTaskRuns[0].id?.wfRunId?.id + '-' + index}
                          value={selectedTaskRuns[0].id?.wfRunId?.id + '-' + index || ''}
                        >
                          <AccordionTrigger
                            className={`py-2 text-xs ${selectedAttempt === selectedTaskRuns[0].id?.wfRunId?.id + '-' + index ? 'text-blue-600' : ''}`}
                          ...
                          <AccordionContent>
                            <div className="space-y-2 pl-2 text-xs">
                              ...
                              <div>
                                <div className="mb-1 text-[#656565]">Result:</div>
                                <ExpandableText
                                  text={
                                    attempt.error
                                      ? JSON.stringify(attempt.error, null, 2)
                                      : attempt.exception
                                        ? JSON.stringify(attempt.exception, null, 2)
                                        : attempt.output
                                          ? getVariableValue(attempt.output)?.toString()
                                          : 'No result'
                                  }
                                  isCode={true}
                                  maxLength={100}
                              ...
                              <div>
                                <div className="mb-1 text-[#656565]">Log Output:</div>
                                <ExpandableText
                                  text={
                                    attempt.logOutput
                                      ? getVariableValue(attempt.logOutput)?.toString()
                                      : 'No log output'
                                  }
                                  isCode={true}
                                  maxLength={100}
          ...
          {activeTab === 'definition' && (
            <div className="p-4">
              <h3 className="mb-2 text-sm font-medium">Node Definition</h3>
        ...
        {!selectedId && (
          <div className="flex flex-1 items-center justify-center p-4 text-xs text-[#656565] italic">

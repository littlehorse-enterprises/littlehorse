...
export const WfSpecTable: FC<SearchResultProps> = ({ pages = [] }) => {
  ...
  const wfSpecs = useMemo(() => {
    ...
    pages.flatMap(page => page.results).reverse().forEach(({ name, majorVersion, revision }: WfSpecId) => {
      if (!specMap.has(name)) {
      ...
      <div className="flex flex-col max-h-[600px] overflow-auto">
        {wfSpecs.map(wfSpec => (
          <Fragment key={wfSpec.name}>
            <button className="items-center px-2 py-2 rounded-md gap-3 flex hover:bg-gray-100" key={wfSpec.name} onClick={() => router.push(`/wfSpec/${wfSpec.name}/${wfSpec.latestVersion}`)}>
              <p>
                {wfSpec.name}
              </p>
              <div className="flex items-center gap-2 rounded bg-blue-200 px-2 font-mono text-sm text-gray-500">
                <TagIcon className="h-4 w-4 fill-none stroke-gray-500 stroke-1" />Latest: v{wfSpec.latestVersion}
              </div>
            </button>
            <Separator />
          </Fragment>

...
export function useNodeDataEditor(nodeId: string): UseNodeDataEditorResult {
  ...
  const currentTaskName = currentNodeData?.node?.$case === 'task' && 
  currentNodeData.node.value.taskToExecute?.$case === 'taskDefId'
  ? currentNodeData.node.value.taskToExecute.value.name 
  : '';

  const currentVarName = currentNodeData?.node?.$case === 'task' && 

...
const FittingCaption: FC<{
  ...
  <span
    className={cn('block max-w-[5.5rem] whitespace-nowrap text-center leading-tight', className)}
    style={{
...
export const DiagramNodeShell: FC<DiagramNodeShellProps> = ({ id, label, icon: Icon, theme, subtitle, children }) => (
  <div className="group relative inline-block">
    <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-0.5 inline-flex -translate-x-1/2 items-center justify-center gap-0.5 whitespace-nowrap">
      ...
      <span
        className={cn('text-center text-[9px] font-semibold uppercase leading-tight tracking-wider', theme.labelClass)}
        title={label}
      >
        {label}
      </span>
    ...
    <span
      className="pointer-events-none invisible absolute left-1/2 top-full z-10 mt-0.5 -translate-x-1/2 whitespace-nowrap pt-0.5 text-center font-mono text-[7px] leading-tight text-slate-500 group-hover:visible"
      style={{ marginTop: subtitle ? '1.225rem' : undefined }}
      title={id}
    >
      {id}
    </span>
...
export const DiagramNodeDiamond: FC<DiagramNodeDiamondProps> = ({
  ...
  <div className={cn('relative grid cursor-pointer place-items-center', sizeClass)}>
    ...
    <div className={cn('absolute', innerInsetClass, theme.bgClass, selected && theme.selectedBgClass, DIAMOND_CLIP)} />

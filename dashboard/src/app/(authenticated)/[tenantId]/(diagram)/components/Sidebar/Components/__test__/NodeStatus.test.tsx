...
jest.mock('../StatusColor', () => {
  ...
  return {
    WF_RUN_STATUS: {
      RUNNING: { color: 'blue', Icon: MockIcon, textColor: 'text-blue-700' },
      COMPLETED: { color: 'green', Icon: MockIcon, textColor: 'text-green-700' },
    },
    TASK_STATUS: {
      FAILED: { color: 'red', Icon: MockIcon2, textColor: 'text-red-700' },
      SUCCESS: { color: 'teal', Icon: MockIcon2, textColor: 'text-teal-700' },
    },
  }
})
...
test('renders workflow status (default type) with correct classes and icon', () => {
  ...
  expect(icon).toHaveClass('h-4', 'w-4', 'stroke-blue-500', 'fill-transparent')
...
test('renders task status when type="task" with correct classes and icon', () => {
  ...
  expect(icon).toBeInTheDocument()
  expect(icon).toHaveClass('h-4', 'w-4', 'stroke-red-500', 'fill-transparent')

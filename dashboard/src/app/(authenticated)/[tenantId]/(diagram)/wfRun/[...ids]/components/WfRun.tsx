...
export const WfRun: FC<{ ids: string[]; tenantId: string }> = ({ ids, tenantId }) => {
  ...
  const { wfRunData: parentWfRunData } = useWfRun({
    wfRunId: wfRunData?.wfRun?.id?.parentWfRunId ?? { id: '', parentWfRunId: undefined },
    tenantId,
  })
  ...
  const inheritedVariables = variableDefs
    .filter(vD => vD.accessLevel === WfRunVariableAccessLevel.INHERITED_VAR)
    .map(vD => {

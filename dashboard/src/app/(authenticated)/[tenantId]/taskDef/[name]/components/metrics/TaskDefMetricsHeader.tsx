...
export const TaskDefMetricsHeader: FC<TaskDefMetricsHeaderProps> = ({
  ...
  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
    <div className="flex items-center gap-3">
      <CardTitle className="text-base font-medium">Task execution metrics</CardTitle>
    ...
    <div className="flex flex-wrap items-center justify-end gap-x-4 gap-y-2">
      <div className="flex items-center gap-2">
        <Label htmlFor="task-metrics-view" className="whitespace-nowrap text-xs text-muted-foreground">
          View
        </Label>
        <Select value={viewMode} onValueChange={v => onViewModeChange(v as ViewMode)}>
          <SelectTrigger id="task-metrics-view" className="h-8 w-[140px] text-xs">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="flex items-center gap-2">
        <Label htmlFor="task-metrics-bucket" className="whitespace-nowrap text-xs text-muted-foreground">
          Bucket size
        </Label>
        <Select value={bucketMinutes} onValueChange={onBucketMinutesChange}>
          <SelectTrigger id="task-metrics-bucket" className="h-8 w-[160px] text-xs">
      ...
      <div className="flex items-center gap-2">
        <Label htmlFor="task-metrics-range" className="whitespace-nowrap text-xs text-muted-foreground">
          Time range
        </Label>
        <Select value={rangeMinutes} onValueChange={onRangeMinutesChange}>
          <SelectTrigger id="task-metrics-range" className="h-8 w-[160px] text-xs">
            <SelectValue />
          </SelectTrigger>
          <SelectContent>

...
export function formatDate(timestamp: TimestampInput): string {
    ...
    try {
        const date = (timestamp as ProtobufTimestamp)?.seconds
            ? new Date((timestamp as ProtobufTimestamp).seconds! * 1000 + ((timestamp as ProtobufTimestamp).nanos || 0) / 1000000)
...
export function getTaskStatusIcon(status: TaskStatus): React.ReactElement {
    if (status === TaskStatus.TASK_SUCCESS) {
        return <CheckCircle className="h-3 w-3 text-green-500 mr-1" />
    } else if (status === TaskStatus.TASK_FAILED || status === TaskStatus.TASK_TIMEOUT ||
        status === TaskStatus.TASK_OUTPUT_SERDE_ERROR || status === TaskStatus.TASK_INPUT_VAR_SUB_ERROR ||
...
export function getTaskStatusBadge(status: TaskStatus): React.ReactElement {
    if (status === TaskStatus.TASK_SUCCESS) {
        ...
        return <Badge variant="outline" className="bg-gray-100 text-gray-800 hover:bg-gray-100">SCHEDULED</Badge>
    }
} 
...
export type TaskRunSearchProps = SearchTaskRunRequest &
  WithTenant &
...
export const searchTaskRun = async ({
  ...
  const requestWithBookmark = bookmarkAsString
    ...
    : req;
  const taskRunIdList: TaskRunIdList = await client.searchTaskRun(
    requestWithBookmark
  );
  ...
  const taskRunWithDetails: runDetails[] = await Promise.all(
    hydrateWithTaskRunDetails()
  );

...
public interface ThreadBuilder {
    /**
     * This is the reserved Variable Name that can be used as a WfRunVariable in
     * an Interrupt Handler or Exception Handler thread.
     */
     ...
     * @param args The input parameters to pass into the Task Run. If the type of an arg
     * is a `WfRunVariable`, then that WfRunVariable is passed in as the argument; otherwise,
     * the library will attempt to cast the provided argument to a LittleHorse VariableValue
     * and pass that literal value in.
     ...
     * This method is used to schedule the reassignment of a UserTask to a userGroup
     * when the specified UserTask user assignment reaches its deadline in seconds.
     *
     * @param userTaskOutput that is currently assigned to a UserGroup.
     * @param deadlineSeconds Time in seconds after which the UserTask will be
     * automatically reassigned to the UserGroup.
     */
     ...
     * This method is used to schedule the reassignment of a UserTask to a userId
     * when the specified UserTask user assignment reaches its deadline in seconds.
     *
     * @param userTaskOutput that is currently assigned to a UserGroup.
     ...
     * Adds a User Task Node, and assigns it to a group of users.
     * @param userTaskDefName is the UserTaskDef to assign.
     * @param userGroup is the User Group (either WfRunVariable or String) to
     * assign the task to.
     * @return a UserTaskOutput.
     */
    public UserTaskOutput assignUserTaskToUserGroup(
     ...
     * @param doIf is the block of ThreadSpec code to be executed if the provided
     * WorkflowCondition is satisfied.
     */
    public void doIf(WorkflowCondition condition, IfElseBody doIf);

    /**
     * Conditionally executes one of two workflow code branches; equivalent to an
    ...
    /**
     * Adds a TASK node to the ThreadSpec.
     * @param userTask is a reference to the UserTaskNode that we schedule the action
     * after.
    ...
    /**
     * Conditionally executes some workflow code; equivalent to an while() statement
     * in programming.
     * @param condition is the WorkflowCondition to be satisfied.
    ...
    /**
     * Adds a SPAWN_THREAD node to the ThreadSpec, which spawns a Child ThreadRun
     * whose ThreadSpec is determined by the provided ThreadFunc.
     * @param threadFunc is a ThreadFunc (can be a lambda function) that defines the
     * logic for the child ThreadRun to execute.
     * @param threadName is the name of the child thread spec.
     * @param inputVars is a Map of all of the input variables to set for the child
     * ThreadRun. If you don't need to set any input variables, leave this null.
     ...
     * the resulting Failure has the specified value, name, and human-readable message.
     * @param output is a literal value (cast to VariableValue by the Library) or a
     * WfRunVariable. The assigned value is the payload of the resulting Failure, which
     * can be accessed by any Failure Handler ThreadRuns.
    ...
    /**
     * Adds an EXIT node with a Failure defined. This causes a ThreadRun to fail, and
     * the resulting Failure has the specified name and human-readable message.
     * @param failureName is the name of the failure to throw.
     * @param message is a human-readable message.
     */
    ...
    /**
     * Registers an Interrupt Handler, such that when an ExternalEvent arrives with
     * the specified type, this ThreadRun is interrupted.
     * @param interruptName The name of the ExternalEventDef to listen for.
     * @param handler A Thread Function defining a ThreadSpec to use to handle the
     * Interrupt.
     */
    public void registerInterruptHandler(String interruptName, ThreadFunc handler);

    /**
     * Adds a SLEEP node which makes the ThreadRun sleep for a specified number of
     * seconds.
     * @param seconds is either an integer representing the number of seconds to
     * sleep for, or it is a WfRunVariable which evaluates to a VariableTypePb.INT
     * specifying the number of seconds to sleep for.
     */
     ...
     * null, then this Failure Handler catches all failures.
     * @param handler is a ThreadFunction defining a ThreadSpec that should be used
     ...
     * @param type is the mutation type to use, for example,
     * `VariableMutationTypePb.ASSIGN`.
     * @param rhs is either a literal value (which the Library casts to a Variable
     * Value), a `WfRunVariable` which determines the right hand side of the

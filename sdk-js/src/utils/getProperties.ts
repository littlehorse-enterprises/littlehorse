 ...
 */
const getProperties = (path: string): Partial<Config> => {
  const fullpath = resolve(path)
  const file = readFileSync(fullpath).toString()
  return file.split('\n').filter(Boolean).reduce<Partial<Config>>((config, line) => {
    const match = /^([^#=]+)(={0,1})(.*)$/.exec(line)
    if (match) {
      const property = match[1].trim() as typeof CONFIG_NAMES[number]
      const value = match[3].trim()
      if (CONFIG_NAMES.includes(property) && !!value) {
        config[property] = match[3].trim()
      }
    }
    return config
  }, {})

...
function createBaseCheckpointId(): CheckpointId {
  return { taskRun: undefined, checkpointNumber: 0 };
}

export const CheckpointId = {
  encode(message: CheckpointId, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.taskRun !== undefined) {
      TaskRunId.encode(message.taskRun, writer.uint32(10).fork()).ldelim();
    }
    if (message.checkpointNumber !== 0) {
      writer.uint32(16).int32(message.checkpointNumber);
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): CheckpointId {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCheckpointId();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): CheckpointId {
    return {
      taskRun: isSet(object.taskRun) ? TaskRunId.fromJSON(object.taskRun) : undefined,
      checkpointNumber: isSet(object.checkpointNumber) ? globalThis.Number(object.checkpointNumber) : 0,
    };
  },

  toJSON(message: CheckpointId): unknown {
    const obj: any = {};
    if (message.taskRun !== undefined) {
      obj.taskRun = TaskRunId.toJSON(message.taskRun);
    }
    if (message.checkpointNumber !== 0) {
      obj.checkpointNumber = Math.round(message.checkpointNumber);
    }
    return obj;
  },

  create(base?: DeepPartial<CheckpointId>): CheckpointId {
    return CheckpointId.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<CheckpointId>): CheckpointId {
    const message = createBaseCheckpointId();
    message.taskRun = (object.taskRun !== undefined && object.taskRun !== null)
      ? TaskRunId.fromPartial(object.taskRun)
      : undefined;
    message.checkpointNumber = object.checkpointNumber ?? 0;
    return message;
  },
};
...
function createBaseBulkJobId(): BulkJobId {
  return { id: "" };
}

export const BulkJobId = {
  encode(message: BulkJobId, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.id !== "") {
      writer.uint32(10).string(message.id);
    }
    return writer;
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): BulkJobId {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseBulkJobId();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): BulkJobId {
    return { id: isSet(object.id) ? globalThis.String(object.id) : "" };
  },

  toJSON(message: BulkJobId): unknown {
    const obj: any = {};
    if (message.id !== "") {
      obj.id = message.id;
    }
  ...
  create(base?: DeepPartial<BulkJobId>): BulkJobId {
    return BulkJobId.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<BulkJobId>): BulkJobId {
    const message = createBaseBulkJobId();
    message.id = object.id ?? "";
    return message;
  },

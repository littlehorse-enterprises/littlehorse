...
- **JS side:** `src/feature-matrix/golden.ts` provides `loadGolden` (strict —
  ...
  golden parses and round-trips. When the wfsdk exists, converting a wfsdk
  matrix todo means: build the equivalent workflow in TS, then
  `expectMatchesGolden(compiled, '<name>')`.
- **Not yet covered by goldens:** structs/StructDefs (needs registered
  ...
  dynamic task names. Add reference workflows as those todos come up.

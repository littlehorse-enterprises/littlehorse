...
async function main() {
  ...
  const nameBefore = strValue(
    await client.getVariable({
      wfRunId: parentRun.id,
      threadRunNumber: 0,
      name: 'name',
    })
  )
  ...
  const childRun = await client.runWf({
    wfSpecName: 'child',
    variables: {},
    parentWfRunId: parentRun.id,
  })
  const childFinished = await waitForRun(childRun.id!)
  const nameAfter = strValue(
    await client.getVariable({
      wfRunId: parentRun.id,
      threadRunNumber: 0,
      name: 'name',
  ...
  const grandChildRun = await client.runWf({
    wfSpecName: 'grand-child',
    variables: {},
    parentWfRunId: childRun.id,
  })
  ...
  await client.putExternalEvent({
    ...
    content: {
      value: {
        oneofKind: 'str',
        str: '{"message": "Hello from grand-child!"}',
      },
    },
  })
  ...
  config.close()
  const succeeded = nameBefore === 'obi-wan' && nameAfter === 'yoda' && grandChildFinished.status === LHStatus.COMPLETED

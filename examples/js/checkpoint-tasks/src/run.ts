...
async function main() {
  const run = await client.runWf({
    wfSpecName: 'example-checkpointed-tasks',
    variables: {
      'input-name': { value: { oneofKind: 'str', str: 'Qui-Gon Jinn' } },
    },
  })
  const finished = await waitForRun(run.id!)
  console.log(`example-checkpointed-tasks -> ${LHStatus[finished.status]} (${run.id!.id})`)
  config.close()

# LittleHorse Arch
LittleHorse is a Microservice Orchestration Engine. For a description of the problem statement, see the LittleHorse Overview. It’s also recommended to read the `PROGRAMMING_MODEL.md` in the lh-sdk repo to get a background for how Workflow logic works.
...
The gRPC server’s API is quite simple and can be seen in `proto/service.proto` from the lh-sdk repository.

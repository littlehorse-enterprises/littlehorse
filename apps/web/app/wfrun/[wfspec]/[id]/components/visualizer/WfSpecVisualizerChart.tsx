...
export const WfSpecVisualizerChart = ({
	data,
	onClick
}: {
	data: any
	...
	const drawChart = useCallback((data: any[]) => {
		...
		let svg = _d3
			...
			.attr(
				'height',
				data.length * 110 < minHeight ? minHeight : data.length * 110
			)
		// .attr("height",height);
		...
		lineG
			...
			.style('stroke', (d, ix) => {
				if (!ix || (d.type === 'NOP' && d.node.outgoingEdges.length === 1))
					return 'transparent'
				return '#B3B3B3'
			})
			.attr('x1', d => {
		...
		nodes
			...
			.attr('d', function (d, i) {
				...
				if (d.px === 'left') px = -150
				if (d.px === 'right') px = 150
				if (d.type === 'ENTRYPOINT')
					return roundedRect(width / 2 + px, d.level * 90 + 65, 120, 56, 28)
				if (d.type === 'EXIT')
					return roundedRect(width / 2 + px, d.level * 90 + 65, 100, 56, 28)
				if (d.type === 'TASK')
					return roundedRect(width / 2 + px, d.level * 90 + 65, 180, 48, 12)
				if (d.type === 'START_THREAD')
					return roundedRect(width / 2 + px, d.level * 90 + 65, 180, 48, 12)
				if (d.type === 'WAIT_FOR_THREAD')
					return roundedRect(width / 2 + px, d.level * 90 + 65, 180, 48, 12)
				return roundedRect(width / 2 + px, d.level * 90 + 65, 97, 48, 12)
			})
			.attr('fill-opacity', d => {
				if (d.type === 'ENTRYPOINT') return 0.2
				if (d.type === 'EXIT') return 0.2
				return 1
			})
			.attr('fill', d => {
		...
		lineG2
			.data(
				data.filter(d => d.type === 'NOP' && d.node.outgoingEdges.length > 1)
			)
			.enter()
			.append('path')
			.attr('class', d => d.name)
			.attr('d', (d, i) => rArrow(width / 2, d.level * 90 + 65, 100, 48, 12))
			.attr('fill', 'none')
			.attr('stroke', '#B3B3B3')
		lineG2
			.data(
			...
			.append('path')
			.attr('class', d => d.name)
			.attr('d', d => lArrow(width / 2, d.level * 90 + 65, 100, 48, 12))
			.attr('fill', 'none')
			.attr('stroke', '#B3B3B3')
		lineG2
			.data(
			...
			.append('path')
			.attr('class', d => d.name)
			.attr('d', d => rbArrow(width / 2, d.level * 90 + 65, 100, 48, 12))
			.attr('fill', 'none')
			.attr('stroke', '#B3B3B3')
		lineG2
			.data(
			...
			.append('path')
			.attr('class', d => d.name)
			.attr('d', d => lbArrow(width / 2, d.level * 90 + 65, 100, 48, 12))
			.attr('fill', 'none')
			.attr('stroke', '#B3B3B3')

		// WAIT_FOR_THREAD
		lineG2
			.data(data.filter(d => d.type === 'WAIT_FOR_THREAD'))
			.enter()
			.append('path')
			.attr('d', d =>
				backArrow(

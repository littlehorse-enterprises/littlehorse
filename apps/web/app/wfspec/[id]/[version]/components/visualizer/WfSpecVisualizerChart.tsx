...
export function WfSpecVisualizerChart({ data, onClick }:{data:any, onClick:(n:any) => void}) {
  ...
  const drawChart = useCallback((data:any[]) => {
    ...
    //ARROWS
    const lineG = svg.append('g').selectAll('g')
    lineG.data(data)
      .enter()
      .append('line')
    ...
    // NOP LINES
    const lineG2 = svg.append('g').selectAll('g')
    ...
    // .data(d.node.waitForThreads.threads)
      .append('path')
      .attr('d', (d) => {
        // d.node.waitForThreads.threads.map( (t:any, ix:number) => {
    
        //     console.log('NODE','#idd'+t.threadRunNumber.variableName,_d3.select('#idd'+t.threadRunNumber.variableName))
        // })
    ...
    //TEXT
    const textGN = svg.append('g').selectAll('g')

...
import type { LH_EXCEPTION } from './internals/FailureInformation'
...
function WfVariable(props: WfVariableProps) {
  const [ wfVariableValue, setVariableValue ] = useState<VariableValue | undefined>()
  ...
  const getVariableData = async (wfRunId: string, threadRunNumber: string, name: string) => {
    ...
    if (res.ok) {
      ...
      setVariableValue(lhVariableValue)
    }
  }
  ...
  useEffect(() => {
    ...
    const variableValueLabel = parseValueByType(wfVariableValue)
    setVariableValueLabel(variableValueLabel)

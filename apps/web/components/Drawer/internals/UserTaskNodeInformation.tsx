...
export function UserTaskNodeInformation({ isWFRun, data, wfRunId, run, setToggleSideBar, setCode }:Props) {
        ...
        <div className=''>
          ...
          <DrawerSection title="Node Data" >
            <div className="grid-3">
              ...
              {orderBy(userTaskRun?.events, [ 'time' ],[ 'desc' ])?.find( e => e.reassigned?.oldUser) ? <p className="drawer__nodeData__header">REASSIGNED TO</p> : null}
              {orderBy(userTaskRun?.events, [ 'time' ],[ 'desc' ])?.find( e => e.reassigned?.oldUser) ? <p className="drawer__nodeData__data">{orderBy(userTaskRun?.events, [ 'time' ],[ 'desc' ])?.find( e => e.reassigned).reassigned.oldUser?.id} {`->`} {orderBy(userTaskRun?.events, [ 'time' ],[ 'desc' ])?.find( e => e.reassigned).reassigned.newUser?.id}</p> : null}

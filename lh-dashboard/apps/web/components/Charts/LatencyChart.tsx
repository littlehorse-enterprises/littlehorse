...
<main>
    <div><div class="dot" style="background:#FDBB45; border:1px solid #EDEEF0"></div><label>Average:</label> <div class="value">{C}</div></div>
    <div><div class="dot" style="background:#FDBB45; border:1px solid #FFCC00"></div><label>Maximum:</label> <div class="value">{D}</div></div>
</main>
`
...
export function LatencyChart({ data, type }:LatencyChartProps) {
    ...
    const drawChart = useCallback((dataForChart:any[], windowType:string) => {
        ...
        groups = dataForChart.map(d => ({
            l1:moment(d.label).format(`MMM DD${windowType==='DAYS_1' ? '' : ','}`),
            l2:moment(d.label).format(`HH:${windowType==='HOURS_2' ? '00' : 'mm'}`),
        }))
        ...
        dotg.data(datam)
            ...
            .attr('stroke', (_d, ix) => {
                const val = ((ix-1) * (maxWBar + gap))+(maxWBar/2)
        ...
        svg.selectAll('.shadow')
            .on('mouseover', () => {
                d3.select(this).transition()
                    ...
                    .duration(250)
                    .style('opacity', 1)
                d3.select('#wf-latency-chart-tooltip').style('visibility', 'visible')
            })
            .on('mousemove', (e,d) => {
            ...
            .on('mouseout', () => {
                d3.select(this).transition()
                    ...
                    .duration(250)
                    .style('opacity', 0)
                d3.select('#wf-latency-chart-tooltip').style('visibility', 'hidden')
            })
    ...
    const setD3 = useCallback((latencyData:any[], windowType:string) => {
        ...
        drawChart(latencyData,windowType)
    },[ drawChart ])
    ...
    return <>
        <div className="relative" id="wf-latency-chart">
            <div className="mcToolTip" id="wf-latency-chart-tooltip" />
        </div>

...
We need to support objects that were put into RocksDB before the new upgrade was released. The flow for 

This will initially result in a lot of `get()`'s to keys that don't exist. However, there are two mitigating factors:
* We have bloom filters on all levels, which prevent 99% of SST file reads in missing cases.
* We will check the new (expected) key first, which means that over time the majority of reads will succeed on the first time.
...
We can extend the `ServerInitializationLog` protobuf to understand when the server was created so that it ignores the double-get pattern for new clusters. This will help avoid a lot of the performance losses except for those users who already have legacy clusters.
...
public interface WfRunGroupedObject {
    ...
    /**
     * This is an enum like StoreableType
     */
    WfRunGroupedObjectType getGroupedObjectType();

    /**
     * Returns the portion of the rocksdb key that goes after the WfRun prefix.
     */
    String getPostWfRunRocksDBKey();
}
```

And in proto (note that this is internal):

```
enum WfrunGroupedObjectType {
    STORED_GETABLE = 0;
    WF_RUN_STORED_INVENTORY = 1;
}
```

Next, we add `WF_RUN_GROUPED_OBJECT` as a new enum for `StoreableType` in `storage.proto`.

...
| "One polyglot suite will drag every toolchain through CI" (Temporal's pain, Avro's death). | The design keeps builds per-SDK: each testee builds with its own toolchain and the runner only shells binaries and compares JSON. Today's report-only job consolidates the two v1 builds into one job for simplicity; the split lands with gating. |

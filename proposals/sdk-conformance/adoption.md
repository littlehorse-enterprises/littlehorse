...
covers how SDKs join, who decides what, and the objections we expect —
each answered from the research rather than optimism. What future versions
can entail lives in [design.md](./design.md#future-versions).
...
Three rungs per SDK, each honest and visible in the matrix. No team is ever
forced up a rung; the matrix simply shows where everyone stands.

| Rung | What it means | Cost to the SDK team |
| --- | --- | --- |
| **not joined** | the SDK's column reads *not joined* — visible, zero shame | none |
| **report-only** | a testee exists and results publish on every run; the ledger holds the todos; nothing blocks that team's merges | a two-verb testee + a ledger file |
| **gated** | the SDK's own CI fails on any drift from its ledger — full membership | flipping one CI flag |

v1 lands with sdk-js and sdk-java both **gated**: the suite ships green by
construction (canon is minted from the reference, and every gap is
...
public, always-current inventory of exactly what their SDK does and doesn't
cover — before accepting any CI obligation.
...
- **Canon changes** (any fixture or manifest change) require a PR with the
  ...
  protocol's meaning. This also answers the solo-maintainer concern: no one
  person can change what "correct" means alone, however automated the
  pipeline.
...
- **Ledger changes** for an SDK are reviewed by that SDK's owner;
  ...
  schedule.
- **Disagreements**: in v1 there is nothing to adjudicate — canon is frozen
  and disagreement means red. The genuine governance question arrives with
  runtime cases and a third SDK; the rule then is: server-visible behavior
  is adjudicated by the server's own records; anything else is a reviewed
  canon change. If consensus can't be reached, the case moves to
  `recommended` rather than blocking anyone — visible disagreement beats
  stalled canon.
- **Custody**: the suite starts under its proposing maintainer's
  stewardship. The intended end state is server-team custody — the natural
  owner of a cross-SDK canon is the team that owns the protocol. That
  transfer is a standing offer, not a precondition.
...
| "The suite will die like CloudEvents'." | CloudEvents was optional and external. This rides each participant's normal CI from day one, and joining costs a two-verb shim — under the adoption-cost ceiling every surviving suite respects. |

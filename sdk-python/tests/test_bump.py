...
class TestLHConfig(unittest.TestCase):
    def test_bump_release(self):
        bump = Bump()
        bump.current = Version.parse("0.1.0")

        self.assertEqual(
            bump.next_version(release="minor", prerelease=None), Version.parse("0.2.0")
        )

        self.assertEqual(
            bump.next_version(release="patch", prerelease=None), Version.parse("0.1.1")
        )

        self.assertEqual(
            bump.next_version(release="major", prerelease=None), Version.parse("1.0.0")
        )
    ...
    def test_raise_error_if_version_is_prerelease_and_pass_release(self):
        bump = Bump()
        bump.current = Version.parse("0.1.0-alpha.1")
    ...
    def test_bump_release_and_prerelease(self):
        ...
        self.assertEqual(
            bump.next_version(release="major", prerelease="alpha"),
            Version.parse("1.0.0-alpha.1"),
        )
        self.assertEqual(
            bump.next_version(release="minor", prerelease="alpha"),
            Version.parse("0.2.0-alpha.1"),
        )
        self.assertEqual(
            bump.next_version(release="patch", prerelease="alpha"),
    ...
    def test_bump_prerelease_to_final(self):
        ...
        self.assertEqual(
            bump.next_version(release=None, prerelease=None), Version.parse("0.1.0")
        )
    ...
    def test_bump_higher_prerelease(self):
        ...
        self.assertEqual(
            bump.next_version(release=None, prerelease="alpha"),
            Version.parse("0.1.0-alpha.2"),
        )

        self.assertEqual(
            bump.next_version(release=None, prerelease="beta"),
            Version.parse("0.1.0-beta.1"),
        )

        self.assertEqual(
            bump.next_version(release=None, prerelease="rc"),
            Version.parse("0.1.0-rc.1"),
        )

        bump.current = Version.parse("0.1.0-beta.1")

...
- Search for UserTaskRun where status is DONE
    ...
    public void test() throws LHApiError, InterruptedException, TestFailure, LHSerdeError {
        ...
        String failWf =
                client.runWf(WF_SPEC_NAME, wfSpecVersion, null, Arg.of("should-fail", true));
        String succeedWf =
                client.runWf(WF_SPEC_NAME, wfSpecVersion, null, Arg.of("should-fail", false));
        wfRunIds.add(failWf);
        ...
        SearchTaskRunResponse failedTasks = searchTaskRuns(FAIL_TASK, TaskStatus.TASK_FAILED);
        assertContainsWfRun(failedTasks, failWf);
        assertNotContainsWfRun(failedTasks, succeedWf);

        SearchTaskRunResponse runningFailTasks = searchTaskRuns(FAIL_TASK, TaskStatus.TASK_RUNNING);
        assertNotContainsWfRun(runningFailTasks, failWf);
        assertNotContainsWfRun(runningFailTasks, succeedWf);
        SearchTaskRunResponse runningSlowTasks = searchTaskRuns(SLOW_TASK, TaskStatus.TASK_RUNNING);
        ...
        SearchTaskRunResponse succeededSlowTasks =
                searchTaskRuns(SLOW_TASK, TaskStatus.TASK_SUCCESS);
        ...
        assertContainsWfRun(
                searchUserTaskRunsUserId("obiwan", UserTaskRunStatus.ASSIGNED), succeedWf);
        assertContainsWfRun(
                searchUserTaskRunsUserId("obiwan", USER_TASK, UserTaskRunStatus.ASSIGNED),
                succeedWf);

        UserTaskRunId userTaskId =
                searchUserTaskRunsUserId("obiwan").getResultsList().stream()
                        .filter(id -> id.getWfRunId().equals(succeedWf))
                        .findFirst()
                        .orElseThrow();

        AssignUserTaskRunResponse assignReply =
        ...
        assertNotContainsWfRun(
                searchUserTaskRunsUserId("obiwan", UserTaskRunStatus.ASSIGNED), succeedWf);
        assertContainsWfRun(
                searchUserTaskRunsUserId("fdsa", UserTaskRunStatus.ASSIGNED), succeedWf);

        AssignUserTaskRunResponse reAssignReply =
                client.getGrpcClient()
                        .assignUserTaskRun(
                                AssignUserTaskRunRequest.newBuilder()
                                        .setUserGroup(
                                                UserGroup.newBuilder().setId("mygroup").build())
                                        .setOverrideClaim(true)
                                        .setUserTaskRunId(userTaskId)
                                        .build());
        System.out.println(LHLibUtil.protoToJson(reAssignReply));
        assertThat(reAssignReply.getCode() == LHResponseCode.OK, "should be able to reassign");
        Thread.sleep(150); // allow remote indexes to propagate
        assertContainsWfRun(
                searchUserTaskRunsUserGroup("mygroup", UserTaskRunStatus.UNASSIGNED), succeedWf);
        assertNotContainsWfRun(
                searchUserTaskRunsUserId("fdsa", UserTaskRunStatus.ASSIGNED), succeedWf);
        ...
        assertContainsWfRun(
                searchUserTaskRunsUserId("yoda", UserTaskRunStatus.ASSIGNED), succeedWf);
        assertNotContainsWfRun(
                searchUserTaskRunsUserGroup("mygroup", UserTaskRunStatus.UNASSIGNED), succeedWf);
        ...
        client.getGrpcClient()
                .completeUserTaskRun(
                        CompleteUserTaskRunRequest.newBuilder()
                                ...
                                .setUserId("yoda")
                                .setResult(
                                        UserTaskResult.newBuilder()
                                                .addFields(
                                                        UserTaskFieldResult.newBuilder()
                                                                .setName("foo")
                                                                .setValue(
                                                                        LHLibUtil.objToVarVal(
                                                                                "bar"))))
                                .build());
        Thread.sleep(150);
    ...
    private void assertNotContainsWfRun(SearchUserTaskRunResponse results, String id) {
        for (UserTaskRunId trid : results.getResultsList()) {
            if (trid.getWfRunId().equals(id)) {
...
class UserTaskForm {
    ...
    @UserTaskField public String foo;
}

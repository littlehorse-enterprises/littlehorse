...
public abstract class Test {
    ...
    public void assertStatus(LHClient client, String wfRunId, LHStatus status)
    ...
    public void assertVarEqual(
            LHClient client, String wfRunId, int threadRunNumber, String name, Object desiredValue)
            throws TestFailure {
    ...
    public SearchTaskRunResponse searchTaskRuns(String taskDefName, TaskStatus status)
            throws LHApiError {
        return client.getGrpcClient()
                .searchTaskRun(
                        SearchTaskRunRequest.newBuilder()
                                .setStatusAndTaskDef(
                                        StatusAndTaskDefRequest.newBuilder()
                                                .setStatus(status)
    ...
    public SearchUserTaskRunResponse searchUserTaskRunsUserGroup(
        ...
        SearchUserTaskRunRequest req =
                SearchUserTaskRunRequest.newBuilder()
    ...
    public SearchUserTaskRunResponse searchUserTaskRunsUserGroup(
            String userGroup, UserTaskRunStatus status) throws LHApiError {
        SearchUserTaskRunRequest req =
                SearchUserTaskRunRequest.newBuilder()
                        .setUserGroup(UserGroup.newBuilder().setId(userGroup).build())
                        .setStatus(status)
    ...
    public SearchUserTaskRunResponse searchUserTaskRunsUserGroup(String userGroup)
            throws LHApiError {
        SearchUserTaskRunRequest req =
                SearchUserTaskRunRequest.newBuilder()
                        .setUserGroup(UserGroup.newBuilder().setId(userGroup).build())
    ...
    public SearchUserTaskRunResponse searchUserTaskRunsUserId(
            String userId, UserTaskRunStatus status) throws LHApiError {
        SearchUserTaskRunRequest req =
                SearchUserTaskRunRequest.newBuilder()
                        .setUser(User.newBuilder().setId(userId).build())
                        .setStatus(status)
    ...
    public void assertTaskOutput(
        ...
        VariableValue expectedVarVal =
                objToVarVal(expectedOutput, "Couldn't convert expected output to varval");

        TaskRun taskRun = getTaskRun(client, nodeRun.getTask().getTaskRunId());
        if (!LHLibUtil.areVariableValuesEqual(
                expectedVarVal, taskRun.getAttempts(taskRun.getAttemptsCount() - 1).getOutput())) {
            throw new TestFailure(
                    this,
    ...
    public NodeRun getNodeRun(
            LHClient client, String wfRunId, int threadRunNumber, int nodeRunPosition)
            throws TestFailure, LHApiError {
        NodeRun result;
        try {
    ...
    public UserTaskRun getUserTaskRun(LHClient client, UserTaskRunId id)
            throws TestFailure, LHApiError {
        UserTaskRun result;
        try {
        ...
        if (result == null) {
            throw new TestFailure(
                    this,
                    "Couldn't find userTaskRun " + id.getWfRunId() + "/" + id.getUserTaskGuid());
        }

        return result;
    }

    public Variable getVariable(LHClient client, String wfRunId, int threadRunNumber, String name)
            throws TestFailure {
        Variable result;
        ...
        if (result == null) {
            throw new TestFailure(
                    this, "Couldn't find variable " + wfRunId + " " + threadRunNumber + " " + name);
        }
    ...
    public List<?> getVarAsList(
            LHClient client, String wfRunId, int threadRunNumber, String varName)
            throws TestFailure {
        VariableValue varVal = getVariable(client, wfRunId, threadRunNumber, varName).getValue();
        try {
            return LHLibUtil.deserializeFromjson(varVal.getJsonArr(), List.class);
        } catch (JsonProcessingException exn) {
            throw new TestFailure(
                    this, "Couldn't deserialize variable " + varName + ":" + exn.getMessage());
        }
    }

    public <T> T getVarAsObj(
            LHClient client, String wfRunId, int threadRunNumber, String varName, Class<T> cls)
            throws TestFailure {
        VariableValue varVal = getVariable(client, wfRunId, threadRunNumber, varName).getValue();
        try {
    ...
    protected ExternalEventId sendEvent(
            LHClient client, String wfRunId, String eventName, Object content, String guid)
            throws TestFailure, LHApiError {
        VariableValue varVal = objToVarVal(content, "Failed converting event input");
        if (guid == null) {
            guid = generateGuid();
        }
        PutExternalEventRequest.Builder req =
    ...
    protected ExternalEvent getExternalEvent(LHClient client, ExternalEventId eventId)
            throws TestFailure, LHApiError {
        ExternalEvent reply;
        try {
            reply =
    ...
    public void assertThreadStatus(
            LHClient client, String wfRunId, int threadRunId, LHStatus status)
            throws TestFailure, LHApiError {
        WfRun wfRun = getWfRun(client, wfRunId);
    ...
    public List<VariableValue> getTaskRunOutputs(
            LHClient client, String wfRunId, int threadRunNumber) throws TestFailure, LHApiError {
        List<VariableValue> out = new ArrayList<>();

        int numNodes =
                getWfRun(client, wfRunId).getThreadRuns(threadRunNumber).getCurrentNodePosition();
    ...
    public void assertTaskOutputsMatch(
            LHClient client, String wfRunId, int threadRunNumber, Object... desiredOutputs)
            throws TestFailure, LHApiError {
        ...
        for (int i = 0; i < desiredOutputs.length; i++) {
            ...
            if (!LHLibUtil.areVariableValuesEqual(desired, actual.get(i))) {
                throw new TestFailure(
                        this, "Node outputs didn't match on the " + i + " th task execution!");
            }
        }
    }
